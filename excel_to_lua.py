...
def write_table(luaT, luaN, outfile = '-', withfunc = True):
    ...
    outfp.write("local xlstable = {\n")
    for sidx, sheet in luaT.iteritems():
        # outfp.write("[%d] = {\n"%(sidx + 1))
        # 第一行是列名称，第二行是列名称的描述；这两行做特殊处理不导出；如果第一个字段tag[0]为空的行也不会导出
        if sheet.has_key(0):
            columnnames = sheet[0]
        for rowidx, row in sheet.iteritems():
            if rowidx!=0 and rowidx!=1:
                # outfp.write("\t[%d] = {\n"%(rowidx + 1))
                if row.has_key(0):
                    if type(row[0]) is int: outfp.write("\t[%d] = {\n"%row[0])
                    else : outfp.write("\t[ [[%s]] ] = {\n"%format_output(row[0]))
                    #else : outfp.write("\t[ [[%s]] ] = {\n"%row[0])

                    # if type(row[0]) is int: outfp.write("\t[%d] = { "%row[0])
                    # else : outfp.write("\t[ [[%s]] ] = { "%row[0])
                    # outfp.write("\t[%d] = {\n"%row[0])
                    for colidx, col in row.iteritems():
                        try:
                            if columnnames.has_key(colidx):
                                if type(col) is int: s = "%d"%(col)
                                elif type(col) is float: s = FLOAT_FORMAT%(col)
                                else : s = "[[%s]]"%(format_output(col))
                                # outfp.write("[ [[%s]] ] = %s, "%(format_output(columnnames[colidx]), s))
                                outfp.write("\t\t[ [[%s]] ] = %s,\n"%(format_output(columnnames[colidx]), s))
                        except Exception, e:
